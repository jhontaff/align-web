...
**Resuelto (2026-09-16)**: la barra de estado del PWA instalado seguía a `prefers-color-scheme` (el SO), no a `data-theme` (lo que el usuario elige en la app) — si forzabas claro con el SO en oscuro, la barra de estado se quedaba oscura. `index.html` pasó a tener **una sola** `<meta name="theme-color">` sin `media`, y el mismo `effect()` que escribe `data-theme` en `ThemeService` (ver arriba) ahora también actualiza su `content` con `Meta.updateTag()` (`@angular/platform-browser`, no manipulación manual del DOM) según `theme()` ya resuelto — así reacciona igual en `system` que si el SO cambia en caliente. El script inline de `index.html` duplica el mismo mapa de colores (`THEME_COLORS`, mismo patrón que la clave `align_theme`) para fijar la meta antes del primer pintado y evitar el flash del color por defecto al abrir la PWA instalada en frío. A diferencia de `data-theme`, esta meta se escribe también en `system`: no hay un `@media` en CSS que la resuelva sola. Presupuesto de `angular.json` subido de 560 kB a 565 kB: el servicio `Meta` de `@angular/platform-browser` (eager, vía `ThemeService`) empujó el bundle inicial 961 bytes por encima del límite anterior.
