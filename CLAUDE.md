...
`features/user/user.service.ts` (2026-09-13): `updateProfile()` (`PUT /me`), `uploadAvatar()`/`deleteAvatar()` (`PUT`/`DELETE /me/avatar`) y el signal `avatarUrl`. `PUT /me/password` y `PUT /me/email` siguen sin consumir — se configuran explícitamente en otra iteración (ver [Current status](#current-status)).
