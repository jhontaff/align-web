...
**La primera versión usaba `Meta.updateTag()` y no funcionaba en un Android real** (confirmado descartando primero caché del Service Worker: el aviso de `AppUpdateService` apareció, se aceptó, la app confirmó estar en la versión nueva, y la barra de estado seguía mal). Causa: `updateTag()` sobre una meta que ya existe hace un `setAttribute('content', ...)` en el nodo — y Chrome/Android **no repinta la barra de estado de una PWA instalada (`display: standalone`) ante esa mutación**, solo la nota si el elemento en sí cambia. Es un bug conocido de Chrome, no de esta app. Arreglo: `this.meta.removeTag('name="theme-color"')` seguido de `this.meta.addTag({ name: 'theme-color', content: ... })` en el mismo efecto, para que cada cambio de tema cree un nodo `<meta>` nuevo en vez de mutar el existente.
