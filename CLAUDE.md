...
- `GET /api/users/me` — requires auth → `ApiResponse<UserResponse>`. Use this to validate a stored token and hydrate the current user on app bootstrap. **Replaces `GET /auth/me`**, removed from the backend on 2026-09-12 when `user` got its own controller — `AuthController` no longer has a `/me` mapping at all.
- `AuthResponse` = `{ accessToken, tokenType: "Bearer", expiresAt }`.
- `UserResponse` = `{ id: uuid, email, firstName, lastName, role, createdAt: instant, hasAvatar: boolean }` (`role`: `USER` | `ADMIN`). `id` is a **UUID string**, not numeric — same pattern as `TaskResponse.id`.
- **The JWT subject is now the user id, not the email** (also 2026-09-12, done as a prerequisite for letting users change their email). Consequence already resolved, not a pending migration: every session alive at that deploy was invalidated once — a legacy token fails to parse and the client gets a clean 401 (clear token, redirect to login), same path as any other auth failure.
...
- **401 from a missing or invalid JWT now follows that shape too**, since the backend added a `JwtAuthenticationEntryPoint` that writes `ApiResponse.error(401, "Authentication required.")` for every auth failure (missing, expired, malformed, invalid signature — all collapsed into the same message). The recommendation doesn't change: **still branch on HTTP status code for auth failures, never on `error.error.message`** — that message is a fixed English string meant for logs, not for display, and there's no confirmation 403 (authorization, not authentication) goes through the same entry point.
...
- **Confirmación de la tool de eliminar** (descubierto 2026-09-12 contra `/v3/api-docs`, no anunciado por el backend en ningún changelog): crear y actualizar (tareas, transacciones, hábitos) el agente los sigue ejecutando directo, pero **eliminar** deja una acción pendiente de confirmación humana en vez de borrar al vuelo. Controlador `pending-action-controller`, sobre el sobre `ApiResponse<T>` **normal** — a diferencia de `/chat` y `/history`, estos tres SÍ pasan por `unwrapInterceptor` sin ninguna excepción:
  - `GET /api/agent/pending-actions` → `ApiResponse<PendingActionResponse[]>`
  - `POST /api/agent/pending-actions/{id}/confirm` → `ApiResponse<Object>` (ejecuta la tool)
  - `POST /api/agent/pending-actions/{id}/reject` → `ApiResponse<Void>` (la descarta sin ejecutarla)
  - `PendingActionResponse = { id: uuid, toolName: string, arguments: object, createdAt: datetime }`. `toolName` es `string` libre en el spec, no un enum cerrado. **Verificado end-to-end 2026-09-12** contra una eliminación real: `toolName` es `delete_task` (**snake_case**, no `deleteTask` como se había asumido antes de probar) y `arguments` hoy solo trae `{ taskId: uuid }` — ni título ni nombre legible. El frontend (`pending-action-labels.ts`) no asume una lista cerrada de tools ni una forma fija de `arguments`, y siempre tiene un fallback genérico ("Se eliminará este elemento…" / sin el nombre entre comillas) para cuando falta cualquiera de los dos.
...
- **Actualizar la app instalada es cosa nuestra, no del navegador** (2026-09-09). El SW de Angular fija cada cliente a la versión con la que arrancó: descubre la versión nueva en una carga y la pinta en la SIGUIENTE. En una pestaña eso pasa solo, porque se cierra y se abre a diario; en una PWA instalada en Android **no** —salir con el botón de inicio no mata el documento, al volver se reanuda el mismo—, así que la app se queda en la versión del día que se instaló. Ese fue el síntoma real: la web actualizada y el icono del móvil no. Cuatro decisiones dentro:
  - **`AppUpdateService` (`core/pwa/`) pregunta en `visibilitychange`**, más un intervalo largo (6 h) como red de seguridad. Volver a primer plano es el único momento fiable en que una PWA instalada reaparece sin recargar; el intervalo solo cubre la pestaña que lleva medio día abierta. Va en `runOutsideAngular` por la razón de siempre: con Zone.js un temporizador vivo deja la zona inestable para siempre y cuelga cualquier `whenStable()`.
  - **Se ofrece con un aviso (`layout/update-banner/`), no se recarga sola.** Recargar en cuanto la versión está lista borra sin avisar el formulario a medio rellenar o el mensaje a medio escribir. La recarga en sí es barata —el token vive en `localStorage` y la sesión se rehidrata—; lo que se pierde es lo que hay en pantalla, y eso no le toca decidirlo al shell. El host del componente es la región `role="status"` y está montado siempre, con el aviso apareciendo dentro: una región viva que entra en el DOM ya con su contenido no se anuncia de forma fiable.
  - **`activateUpdate()` y `location.reload()` van SIEMPRE juntos.** Activar sin recargar deja la página vieja corriendo mientras el SW ya sirve archivos nuevos: el primer `import()` de una ruta lazy pide un chunk cuyo hash ya no existe y falla. Con todas las rutas en `loadComponent`, eso no es hipotético. El mismo fallo por la otra vía —despliegue con la pestaña abierta, sin SW controlándola— lo cubre `reloadOnStaleChunk`, ver [Features de provideRouter](#features-de-providerouter-habilitadas).
  - **Y la otra mitad del arreglo es de servidor.** `nginx.conf` no declaraba ningún `Cache-Control`, y sin él el navegador aplica caché heurística. Sobre `index.html` eso rompe la actualización sin dar ningún error visible: el SW descarga el manifiesto nuevo, pide el index, recibe el viejo, el hash no cuadra y la instalación de esa versión falla en silencio. Ahora `index.html`, `ngsw.json`, `ngsw-worker.js` y el manifest van con `no-cache`, y todo lo que lleva hash en el nombre con `immutable` a un año. `/api` y `/auth` pasaron a `^~` porque en nginx una regex gana a un prefijo normal y `/api/algo.js` habría acabado en la regex de estáticos en vez de en el backend. Falta comprobar que Nginx Proxy Manager no tenga *Cache Assets* activado: sería otra capa por delante.
...
- **El breakpoint, el bug que causó, y por qué ya no se apaga nunca** (corregido 2026-09-05, extendido el mismo día). Gridster decide si está en "modo móvil" comparando `mobileBreakpoint` contra un ancho que mide él, y **en modo móvil `canBeDragged()` devuelve `false` pase lo que pase**. La primera versión usaba `useBodyForBreakpoint: true` + 1024, o sea `document.body.clientWidth` — que **excluye la barra de scroll**— contra el mismo número que la media query de `_layout.scss`, que compara contra el viewport y **la incluye**. Dos varas de medir para el mismo umbral: había una franja en la que el CSS pintaba el escritorio, con su botón de "Personalizar", y el arrastre estaba muerto sin ningún aviso.
  El primer arreglo sincronizaba el umbral con `BreakpointService` en vez de dejar que gridster lo midiera — mejor, pero seguía dejando entrar a gridster en modo móvil por debajo de 1024, y ahí el arrastre sigue muerto por diseño de la librería. Como la siguiente petición fue justo **personalizar también en el teléfono**, ese modo móvil dejó de ser una opción: `mobileBreakpoint: 0` es ahora un centinela fijo **para siempre**, nunca se vuelve a tocar. Gridster no entra en su modo `.mobile` en ningún ancho.
  Lo que sí reacciona al ancho es `minCols`/`maxCols`: `DASHBOARD_COLUMNS` (12) en escritorio, `DASHBOARD_MOBILE_COLUMNS` (1) por debajo, escrito por el mismo `effect` sobre `BreakpointService.isDesktop()` de antes — sigue siendo el único `matchMedia` de verdad, el que también lee el SCSS. Con una columna, gridster sigue colocando de verdad (no hay pila por CSS ni `height: auto !important`): cada tarjeta ocupa el 100% del ancho y el arrastre reordena, que es justo lo que el modo móvil de la librería nunca habría permitido.
- **Personalizar también en el teléfono, en una sola columna** (2026-09-05). Dos consecuencias de lo anterior:
  - **Escritorio y móvil son dos disposiciones independientes**, cada una con su propio guardado (`DashboardLayoutVariant`, ver el bullet de persistencia) y su propio botón "Restablecer" — restablecer en el teléfono no toca lo que el usuario colocó en el monitor grande, y viceversa. `Overview.applyColumnLayout()` es quien conmuta las dos al cruzar el umbral, escribiendo directamente en los mismos siete objetos (`cards`) que gridster ya tiene enlazados — nunca se cambia la identidad del objeto, solo los números que contiene, por la misma razón que `cards` no es un `signal`.
  - **El redimensionado se apaga entero en móvil**, aunque el modo edición esté activo (`toggleEditing()` y `applyColumnLayout()` comparten esa condición, para que cruzar el breakpoint con el modo edición ya abierto no deje un redimensionado colgado). Con una columna no hay ancho que repartir y el alto ya no es del usuario en las tres tarjetas `--fill` — personalizar en el teléfono es reordenar, nada más.
  - `dashboard-layout.spec.ts` (8 pruebas) y dos `describe` nuevos en `gridster-drag.spec.ts` cubren esto: que la disposición de móvil apila en el orden de `DASHBOARD_CARD_IDS` reutilizando el `rows` de escritorio, que guardar una variante no toca la otra, y que una rejilla de una sola columna **no** activa el modo `.mobile` de gridster ni impide arrastrar — la prueba que habría fallado si alguien reintrodujera `mobileBreakpoint` como detector de pantalla estrecha.
- **Alto automático de una sola tarjeta.** "Últimos movimientos" crece con su contenido; las otras seis tienen alto determinista. Hasta el 2026-09-05 eran dos —"Gastos por categoría" también—, ver el bullet siguiente. Tres cosas que no se ven en el código:
  ...
  - Y por eso la tarjeta automática **no se redimensiona en vertical** (`handlesFor()`): si pudiera, el usuario la estiraría, llegarían datos y el observador la devolvería a su tamaño. Dos fuerzas escribiendo el mismo número.
- **Una tarjeta de contenido elige una de dos relaciones con su celda, y se declara.** O **alto automático** (hoy solo `recent`): manda el contenido y la celda se ajusta. O **`.dashboard__card--fill`** (`byCategory`, `pace`, `flow`): manda la celda y el contenido se reparte ese alto — dentro de la tarjeta, lo único que se estira es el área de trazado o la lista (`flex: 1 1 $plot-height` en vez de `block-size` fijo), porque todo lo demás mide lo que mide su texto y estirarlo solo haría huecos. Es lo que hace que redimensionar la tarjeta sirva para algo. Precisiones:
  ...
  - **`--fill` no lleva `overflow`.** `monthly-flow` deja su área de trazado en `overflow: visible` para que el emergente de cada columna se salga por arriba, y un contenedor de scroll lo cortaría. `expense-by-category` es la excepción con motivo propio: su `<ul class="chart">` pone `overflow-y: auto` **sobre sí misma**, no sobre la celda, así que una lista que no quepa hace scroll interno en vez de recortarse o desbordar.
  - **Y no se pone nunca sobre la tarjeta de alto automático** (`recent`): `block-size: 100%` haría que el `ResizeObserver` midiera el alto que la rejilla acaba de imponer — el bucle de realimentación.
  - **"Gastos por categoría" pasó de alto automático a `--fill` el 2026-09-05.** Antes crecía con el número de categorías del periodo y nunca hacía scroll; ahora es una tarjeta de tamaño fijo y redimensionable, como `pace`/`flow`, y el precio es el `overflow-y: auto` del punto anterior — con pocas categorías sobra hueco debajo de la lista, con muchas hay que deslizar dentro de la tarjeta para verlas todas. Es la pieza que faltaba para que las tres cifras y los tres gráficos compartan el mismo idioma de personalización; "Últimos movimientos" se quedó fuera a propósito, sin que nadie lo pidiera cambiar.
  - Las declaraciones que ponen al host de `spending-pace`, `monthly-flow` y `expense-by-category` en columna flexible viven todas en `overview.scss`, no en un `:host` de cada componente: dónde está montado no es una propiedad del componente. Fuera del panel, los tres se comportan igual que siempre.
...
- **Persistencia en `localStorage`, una clave por variante** (`align_finance_layout` para escritorio, `align_finance_mobile_layout` para móvil, 2026-09-05), porque **no hay endpoint de preferencias** en el backend. Las dos comparten forma y validación (`version` + ids conocidos: un layout de otra versión o con tarjetas que no cuadran se descarta entero en vez de dejar alguna sin colocar), pero son independientes — mover algo en el teléfono no reescribe lo guardado para el escritorio. El alto de la tarjeta automática no se guarda en ninguna de las dos.
...
- **Coste medido** (`ng build`): el chunk lazy `overview` pasa de 8.71 kB a 24.44 kB transferidos; el bundle inicial sube 2.5 kB (iconos nuevos y `_gridster.scss` — gridster **no** entra en ningún chunk inicial, comprobado sobre `dist/`).
...
## `gridster-drag.spec.ts` y `dashboard-layout.spec.ts`, y por qué existen

Diez pruebas en `features/finance/overview/gridster-drag.spec.ts` (seis del arrastre de escritorio, cuatro del de una sola columna añadidas el 2026-09-05), escritas **después** de que las tarjetas no se dejaran mover y ni el build ni el tipado dijeran nada. Fijan el contrato con la librería, no la implementación de la pantalla:

- Que la rejilla no se cree móvil aunque la ventana sea estrecha, que `options.api` existe (sin ella, mutar las opciones no hace nada), que el arrastre está apagado fuera del modo edición y encendido dentro, que arranca desde el **cuerpo** de la tarjeta, y que un elemento con `dashboard__no-drag` no se convierte en arrastre.
- Que una rejilla configurada con `minCols`/`maxCols` en 1 (el móvil) sigue sin activar el modo `.mobile` de gridster y sigue dejando arrastrar — la prueba que habría fallado si el arreglo de "personalizar en el teléfono" hubiera vuelto a apoyarse en `mobileBreakpoint` en vez de en el número de columnas.
...
`features/finance/dashboard-layout.spec.ts` (8 pruebas, 2026-09-05) cubre las funciones puras de `dashboard-layout.ts` sin gridster ni DOM: que la disposición de escritorio reparte las tres cifras en fila, que la de móvil apila en el orden de `DASHBOARD_CARD_IDS` reutilizando el `rows` de escritorio sin recalcularlo, y que guardar/cargar una variante no toca la otra ni resucita un layout con una forma distinta a la de hoy.

## Sin verificar todavía

`ng build` limpio, `tsc --noEmit` limpio y la suite en 133/134 (el fallo es previo: falta el proveedor de `SwPush` en `app.spec.ts`). El arrastre —en escritorio y en una sola columna— sí está verificado en un navegador real por las pruebas de arriba. **Lo que sigue sin verse a mano**: el resultado contra el backend vivo, el crecimiento de "Gastos por categoría" y su scroll interno con datos reales, el tema oscuro, y sobre todo la personalización en móvil de verdad — en un teléfono, con el dedo, no con `MouseEvent` sintéticos.
...
- **`ThemeToggle` es icon-only** (`.btn .btn-ghost .btn-icon`), visible en el header en todos los tamaños de pantalla. Tuvo una segunda presentación (`variant="menu"`, fila del desplegable de `session-menu`) hasta que ese componente se eliminó el 2026-09-13 — ver la nota en [Decisiones abiertas](#decisiones-abiertas).

**Resuelto (2026-09-16, corregido el mismo día tras probar en Android real)**: la barra de estado del PWA instalado seguía a `prefers-color-scheme` (el SO), no a `data-theme` (lo que el usuario elige en la app) — si forzabas claro con el SO en oscuro, la barra de estado se quedaba oscura. `index.html` pasó a tener **una sola** `<meta name="theme-color">` sin `media`, y el mismo `effect()` que escribe `data-theme` en `ThemeService` (ver arriba) ahora también actualiza esa meta según `theme()` ya resuelto — así reacciona igual en `system` que si el SO cambia en caliente. El script inline de `index.html` duplica el mismo mapa de colores (`THEME_COLORS`, mismo patrón que la clave `align_theme`) para fijar la meta antes del primer pintado y evitar el flash del color por defecto al abrir la PWA instalada en frío. A diferencia de `data-theme`, esta meta se escribe también en `system`: no hay un `@media` en CSS que la resuelva sola.

**La primera versión usaba `Meta.updateTag()` y no funcionaba en un Android real** (confirmado descartando primero caché del Service Worker: el aviso de `AppUpdateService` apareció, se aceptó, la app confirmó estar en la versión nueva, y la barra de estado seguía mal). Se probó luego `removeTag()` + `addTag()` (recrear el nodo en vez de mutarlo) — **tampoco funcionó**, y ahí se encontró la causa real, no un bug de esta app: investigado y confirmado por fuentes externas.

**Límite real de la plataforma, no arreglable desde el código de la página**: Android instala una PWA como **WebAPK**, y ese WebAPK toma el color de la barra de estado del sistema **una sola vez, del `theme_color` estático de `manifest.webmanifest`**, en el momento en que Android genera el WebAPK — no del `<meta name="theme-color">` en tiempo de ejecución. Cambiar la meta en caliente (con `updateTag`, con `removeTag`+`addTag`, o con cualquier otra técnica) no tiene ningún efecto sobre esa barra concreta mientras la app corre instalada en `standalone`, porque Android directamente no la mira ahí. La meta dinámica sigue teniendo efecto real en otros contextos (una pestaña normal de Chrome, Safari/iOS al añadir a inicio), así que no se revirtió — pero no podía resolver el síntoma reportado.

Decisión tomada (2026-09-16): `manifest.webmanifest` fija `theme_color` en `#0c1324` (el oscuro) de forma permanente — la barra de estado de la PWA instalada **ya no sigue ni al tema de la app ni al del SO**, siempre sale oscura. Es la alternativa elegida frente a quitar `theme_color` del manifest (que habría vuelto a seguir al SO, el comportamiento original que motivó toda esta investigación). El valor por defecto de la meta `theme-color` en `index.html` (antes de que corra el script inline) se sincronizó al mismo `#0c1324` por consistencia, aunque ese valor solo importa fuera del WebAPK instalado.

**Advertencia para el día del deploy**: el `theme_color` de un WebAPK ya instalado no se actualiza con solo desplegar el manifest nuevo — Android revalida el manifest de forma periódica y con retraso (puede tardar días), o requiere desinstalar y reinstalar la PWA. No es un fallo del despliegue si una instalación vieja sigue mostrando el color anterior una temporada.

Presupuesto de `angular.json` subido de 560 kB a 565 kB: el servicio `Meta` de `@angular/platform-browser` (eager, vía `ThemeService`) empujó el bundle inicial 961 bytes por encima del límite anterior.
...
`.menu-item` / `.menu-divider` (2026-08-20) son el caso de libro: nacieron compartidas entre dos componentes con encapsulación distinta (`session-menu` y la variante `menu` de `theme-toggle`, ambos eliminados el 2026-09-13 al consolidar tema/sesión en `/profile`), y la alternativa a la clase global era perforar la encapsulación con `::ng-deep` —deprecado— para compartir cuatro declaraciones. Hoy vive de la misma razón de fondo (pintura sobre un elemento que el consumidor ya escribe), con `features/user/profile/` como consumidor.
...
- `features/<name>/` is self-contained: its own `models/` subfolder for feature-specific DTOs, a service at the feature root, and one subfolder per screen (`task-list/`, `task-form/`). Nothing inside a feature folder should be imported by another feature — if that need shows up, the shared piece moves to `core/`, it doesn't get imported cross-feature. **Excepción direccional, no exclusiva de un solo caso**: una pantalla cuyo propósito completo es *agregar* dominios ajenos para mostrarlos —nunca editarlos ni poseer su lógica— puede importar **servicios y modelos** de esas features, nunca sus componentes, y nunca al revés. Hoy aplica a `features/home/` (el dashboard) y a `features/user/profile/` (estadísticas de hábitos y tareas en una pantalla de perfil de solo lectura, 2026-09-13). La alternativa —subir `TaskService`/`HabitService` a `core/`— convertiría `core/` en el vertedero de toda la lógica de negocio solo para satisfacer la regla. La flecha va siempre `agregador → feature`; si alguna vez una feature necesita algo de un agregador, eso significa que la pieza no era del agregador. No es licencia para cualquier importación cruzada: el criterio es "agregar para mostrar", no conveniencia.
...
│   ├── pwa/                     app-update.service.ts  ← detectar despliegue nuevo      [hoy]
│   │                            reload-on-stale-chunk.ts  ← chunk que ya no existe    [hoy]
...
└── features/
    ...
    └── user/                    user.service.ts ← PUT /me, avatar                [hoy]
                                 profile/  ← identidad + estadísticas, solo lectura [hoy]
                                 profile-edit/  ← nombre + foto; correo/contraseña, después [hoy]
```

`features/user/user.service.ts` (2026-09-13): `updateProfile()` (`PUT /me`), `uploadAvatar()`/`deleteAvatar()` (`PUT`/`DELETE /me/avatar`) y el signal `avatarUrl`. `changePassword()` (`PUT /me/password`) se sumó el 2026-09-16 (ver [Current status](#current-status)). `PUT /me/email` sigue sin consumir — se configura explícitamente en otra iteración.
...
- **Orden de `z-index` del cromo, de menor a mayor**: header 30, `chat-panel` 50, `bottom-nav` 60. La barra va por encima del chat a propósito: con el panel abierto a pantalla completa, las pestañas son la forma de salir de la conversación.
...
- **`BreakpointService` se construyó el 2026-08-20**, y no para elegir el layout —eso sigue siendo CSS— sino para el primer caso que CSS de verdad no podía: **cerrar el `<dialog>` de `session-menu` al cruzar a escritorio** (ese consumidor se eliminó el 2026-09-13; hoy lo usan `finance/overview`, `calendar-widget` y `onboarding-tour.service`, con la misma razón de fondo: algo que CSS no puede decidir por sí solo).
...
- **`layout/pending-delete-confirm/` es un tercer montaje** (2026-09-12), y uno distinto en su forma: no pinta la conversación, es la confirmación modal de una eliminación propuesta por el agente (ver [Chat agent](#chat-agent-apiagent)). Va en `layout/` y no dentro de `ChatPanel` a propósito — **tiene que aparecer aunque el panel esté cerrado**, porque no depende de que el usuario esté mirando la conversación en ese momento; depende de que exista una `pending-action` sin resolver. Reusa `shared/ui/confirm-dialog/` (el mismo `<dialog>` que usa `task-list.ts` para borrar una tarea, `tone="danger"`) en vez de un card nuevo dentro del hilo — es exactamente el mismo patrón "acción destructiva, confirma o cancela", y construir un idioma de UI nuevo solo para esto habría sido la abstracción prematura que [Development philosophy](#development-philosophy) descarta. Se monta en `app.html` como hermano de `<app-chat-panel />`.
...
- **Manejar `pushsubscriptionchange` en el Service Worker** (2026-09-09). El push service del navegador puede **rotar** la suscripción, y lo avisa con ese evento dentro del SW. `ngsw-worker.js` no lo maneja, y un SW propio que lo añadiera (`importScripts('./ngsw-worker.js')` + listener) tampoco podría cerrar el círculo: **el Service Worker no tiene el JWT** —no lee `localStorage`— y `POST /api/notifications/subscribe` exige `Authorization`. Las tres salidas son del mismo tipo:
  - Mover el token a IndexedDB para que el SW lo lea. Amplía la superficie de almacenamiento del token y contradice de frente la decisión de `localStorage`; se descarta.
  - Que el SW haga `postMessage` a un cliente abierto y sea la página quien re-suscriba con su token. Sólo funciona si hay pestaña abierta, y ese caso ya lo cubre la mitigación de abajo en la siguiente apertura.
  - **Que el backend exponga una re-suscripción sin auth, identificada por el `endpoint` viejo.** Es la única que lo resuelve de verdad, y es la que no existe.

  **Mitigación puesta mientras tanto, y es del frontend**: `PushService.syncSubscription()` reenvía la suscripción que el navegador ya tiene en **cada arranque autenticado** (llamado desde el `tap` de `AuthStateService.hydrateUser()`, que es el único punto por el que pasan login, registro y bootstrap, y sólo en éxito — o sea, con token válido para el POST). El backend deduplica por `endpoint`, así que en el caso normal es un no-op. Con esto una rotación se repara en la siguiente apertura de la app en vez de no repararse nunca. Cubre además el otro camino al mismo estado roto: un `subscribe()` que funcionó en el navegador con el `POST /subscribe` caído, que es justo donde deja las cosas el `catch` de `enable()`.

- **`PushService` no usa `SwPush` para suscribir/desuscribir/leer estado — usa `navigator.serviceWorker.ready` + la API cruda de `pushManager`** (2026-09-09). No reintroducir `swPush.requestSubscription()` / `swPush.subscription` / `swPush.unsubscribe()`: todos esperan a que el SW **controle** la pestaña (`navigator.serviceWorker.controller` no nulo), y eso no pasa en la primera carga tras un despliegue ni en la PWA recién instalada hasta recargar. `requestSubscription()` en ese estado **se cuelga sin resolver ni rechazar** — el síntoma real fue el botón "Activar" clavado en "Activando..." en PROD de escritorio, en todos los navegadores, sin que llegara ninguna petición a `fcmregistrations.googleapis.com` (o sea, `pushManager.subscribe()` ni se llamaba). `navigator.serviceWorker.ready` pide sólo un SW **activo**, que es la condición real. Coste: un `decodeVapidKey()` propio (base64url → `Uint8Array`, ~4 líneas) que `SwPush` hacía por dentro, y que `_subscribed` se siembre con una lectura única al arrancar (`readSubscribed()`) en vez de un observable — los mutadores lo mantienen al día después. `withTimeout()` sobre `ready` cubre el caso peor (un SW que no se activa nunca, donde `ready` tampoco resuelve): 15 s → mensaje, no cuelgue. `showTest()` y `syncSubscription()` ya seguían este patrón. **Lo que sigue en `SwPush`**: `isEnabled` (la puerta de "¿hay push?"), `notificationClicks` y `messages` en `listen()` — esos sí requieren un SW en marcha por naturaleza (un push llegando lo implica).
  Si el botón se cuelga y el timeout salta, el fallo está **más abajo**: el SW no se activa en PROD. Sospechoso conocido — **Nginx Proxy Manager con *Cache Assets* activado**, sirviendo `ngsw.json` y los chunks descoordinados (ver la nota de `nginx.conf` en [Actualizar la app instalada](#actualizar-la-app-instalada-es-cosa-nuestra-no-del-navegador)).
...
**Superado 2026-09-13 — `session-menu` se eliminó.** El bloque que sigue documenta por qué existía y las lecciones de `<dialog>` que costó aprender, útiles si algún día vuelve a hacer falta un cajón modal en algún sitio — pero el componente en sí ya no está: tema y cerrar sesión se consolidaron en `/profile` (ver [Chat](#chat-dominio-en-features-montaje-en-layout) y el cambio de 2026-09-13 en [Current status](#current-status)), alcanzable desde el avatar del header en **todos** los tamaños de pantalla, no solo en escritorio. `ThemeToggle` volvió a ser icon-only (perdió su variante `menu`, que era el único consumidor de `.menu-item` fuera de `/profile`).
...
Lo que quedaba en `session-menu.ts` (eliminado, ver la nota de arriba) eran cuatro métodos y un `effect`. Detalles que no eran obvios leyendo el código, y siguen valiendo si vuelve a hacer falta un `<dialog>` modal:
...
- **`withNavigationErrorHandler(reloadOnStaleChunk)`** (2026-09-09, era el hueco anotado aquí hasta que hubo despliegues de verdad). Todas las rutas son `loadComponent` y `outputHashing: "all"` mete el hash en el nombre del archivo, así que el `main.js` que el navegador ya tiene cargado conoce los nombres de chunk de SU build: tras un despliegue, el archivo viejo no está desactualizado, **no existe**. Por defecto el router emite `NavigationError`, nadie lo escucha y la navegación se aborta — el usuario pulsa "Finanzas" y no pasa nada, sin error ni pista. Tres precisiones:
  - **No se busca `ChunkLoadError`**: ese nombre es de webpack, y Angular 20 compila con esbuild sobre `import()` nativo. Lo que llega es un `TypeError` cuyo mensaje cambia por motor ("Failed to fetch dynamically imported module" en Chrome, "error loading dynamically imported module" en Firefox, "Importing a module script failed" en Safari), así que se filtra por mensaje y con los tres.
  - **`location.assign(event.url)`, no `reload()`.** El usuario pidió ir a otra pantalla; recargar repetiría la actual, que es "no pasó nada" otra vez y con un parpadeo.
  - **Centinela en `sessionStorage` contra el bucle.** Si tras recargar la misma URL vuelve a fallar no era un despliegue, es que el chunk no se puede traer (sin red, despliegue a medias): reintentar sería un ciclo de recargas infinito, peor que la navegación muerta que esto arregla.

  Con el Service Worker sano esto no llega a pasar —fija cada cliente a su versión y le sirve sus chunks desde caché aunque el servidor los haya borrado—, así que es la red de seguridad de la red de seguridad: cubre el hueco de cuando el SW todavía no controla la página, el navegador desalojó su caché, entró en modo seguro tras una instalación fallida, o no hay SW en absoluto.
...
- Recarga ante un chunk que ya no existe (2026-09-09) — `core/pwa/reload-on-stale-chunk.ts` registrado con `withNavigationErrorHandler()` en `app.config.ts`. Ver la entrada en [Features de provideRouter](#features-de-providerouter-habilitadas). **Sin pruebas y sin verificar**: no se ha reproducido ni una vez, así que hoy es código que se cree correcto por lectura. Cómo comprobarlo cuando toque, que no es evidente:
  1. Abrir la app desplegada y quedarse en Inicio, **sin visitar** la sección que se va a usar de conejillo (Finanzas, por ejemplo). Si ya se visitó, su chunk está en la caché del SW y no hay nada que romper.
  2. DevTools → Application → Service Workers → marcar **Bypass for network**. Sin esto no se reproduce nada: el SW sano sirve los chunks viejos desde su caché aunque el servidor los haya borrado, que es justamente por lo que este fallo es raro.
  3. Desplegar de nuevo (basta con que cambien los hashes) **sin tocar la pestaña**.
  4. Pulsar "Finanzas". Sin el handler, no pasa nada y en consola queda un `TypeError` de módulo dinámico; con él, la app recarga sola y aterriza en `/finance`.
  5. Para ver el centinela: repetir con el servidor caído. Debe recargar **una** vez y luego quedarse quieta, no entrar en bucle. Se resetea borrando `align_chunk_reload` de `sessionStorage`.
- Actualización de la PWA instalada (2026-09-09) — `core/pwa/app-update.service.ts` (detección en `visibilitychange` + intervalo de 6 h, `unrecoverable` → recarga) y `layout/update-banner/` (aviso con botón, montado en el shell **fuera** del `@if` de sesión para que también sirva en login), más las cabeceras `Cache-Control` de `nginx.conf`. `ng build` limpio y la suite en 177/177 — el fallo previo de `app.spec.ts` se fue con ello: le faltaba `provideServiceWorker(..., { enabled: false })`, que ahora hace falta doblemente porque el shell inyecta `SwPush` y `SwUpdate`. **Sin verificar contra un despliegue real todavía**: hay que desplegar dos veces seguidas para ver el aviso aparecer, y la sintaxis de `nginx.conf` no se ha pasado por `nginx -t`. El presupuesto de bundle inicial subió de 500 a 550 kB en `angular.json`: el cambio añade 2,8 kB sobre 498,65 y 500 era el default genérico del CLI, nunca ajustado a este repo (el tope de error sigue en 1 MB).
...
- App header (`layout/app-header/`) — marca, acciones en línea (`ThemeToggle` + cerrar sesión, solo escritorio) y un círculo de avatar hacia `/profile` (todos los tamaños, ver la entrada de Perfil más abajo). No contiene enlaces de navegación (se fueron a `nav-links.ts` + las dos navs), así que su contenedor de acciones no es un `<nav>`: marcarlo así le mentiría al lector de pantalla, que lo ofrece como punto de referencia navegable. Alto fijo `$header-height`; quien se pega arriba es el envoltorio `.app-shell__top`, no el header por su cuenta. `layout/session-menu/` (el cajón deslizante que sustituía a las acciones en móvil) se eliminó el 2026-09-13 — ver [Decisiones abiertas](#decisiones-abiertas).
...
- Confirmación de eliminar propuesta por el agente (2026-09-12) — `ChatStore` gana un signal `pendingActions` y los métodos `confirmAction()`/`rejectAction()`; `layout/pending-delete-confirm/` es el tercer montaje del dominio (ver [Chat](#chat-dominio-en-features-montaje-en-layout)), reusando `shared/ui/confirm-dialog/`. `pending-action-labels.ts` arma el mensaje ("Se eliminará la tarea de forma permanente.") a partir de `toolName` + un identificador best-effort en `arguments`. **Verificado end-to-end contra el backend vivo** con un script de Playwright (login con token real, tarea real, `"borra la tarea de..."` por el chat, diálogo confirmado abriéndose solo tras la respuesta del agente) — no fue trivial: la primera pasada pareció fallar porque el agente tarda entre 3 y 8 segundos en responder (llamada a LLM de por medio) y un `wait` corto en la prueba se rindió antes de que la respuesta llegara. De ahí salieron dos correcciones reales:
  - `toolName` es `delete_task` (**snake_case**), no `deleteTask` como se había asumido en el diseño sin datos reales — `pending-action-labels.ts` corregido.
  - `arguments` hoy solo trae `{ taskId: uuid }`, sin título — el mensaje del diálogo salía genérico ("la tarea"/"la transacción"/"el hábito") y no nombraba el elemento específico.
  Crear y actualizar desde el chat siguen sin confirmación, sin cambios.
- **Nombre real en el diálogo de eliminar** (2026-09-12, mismo día, tras confirmar el hueco de arriba) — `layout/pending-delete-confirm/resolve-delete-target.ts` pide el nombre legible al servicio dueño del dato (`TaskService.get()`/`TransactionService.get()`/`HabitService.get()` según `toolName`), porque `arguments` no lo trae. Cruza de `layout/` a **tres** features (`tasks`, `finance`, `habits`), no solo a `features/chat/` como el resto del montaje — deliberado y documentado: es el mismo principio que ya permite a `layout/chat-panel/` inyectar `ChatStore` (el shell compone features, las features no se importan entre sí), aplicado tres veces porque el shell necesita *identificar* un dato ajeno, no mostrar ni reutilizar el componente de otra feature. `PendingDeleteConfirm.resolvedIdentifier` sigue el mismo patrón que `TaskDetail.state` (`toObservable` → `distinctUntilChanged` por id → `switchMap` → `toSignal`), no un `effect()` escribiendo otro signal. El diálogo se abre de inmediato con el mensaje genérico y se afina solo en cuanto responde el `GET` — no espera a tenerlo para aparecer. Un fallo del `GET` (p. ej. el elemento ya no existe) cae a `null` y se queda en el genérico, no revienta el diálogo. **Verificado end-to-end con Playwright contra el backend real**: el mensaje pasa de "Se eliminará la tarea de forma permanente." a "Se eliminará la tarea «Podar el jardin» de forma permanente." en cuanto responde `TaskService.get()`.
...
- Perfil de usuario (2026-09-13) — `GET /api/users/me` reemplaza a `GET /auth/me` (eliminado del backend, ver [Auth](#auth)); `UserResponse` corregido (`id` UUID string, + `createdAt`/`hasAvatar`). Pantalla nueva `features/user/profile/` (solo lectura: identidad, tres estadísticas —mejor racha histórica de hábitos, tareas completadas, % con racha activa—, toggle de notificaciones vía `PushService`, fila de apariencia que reusa `ThemeService.cycle()`, fila "Gestión de módulos" que enlaza a `/finance` en vez de a un Home que no tiene layout personalizable, cerrar sesión), en una sola plantilla responsive (`grid-template-areas`, 1 columna en móvil / 2 en escritorio). Círculo de avatar nuevo en `app-header`, visible en todos los tamaños, que navega a `/profile`. Sin `user.service.ts` todavía (ver el árbol canónico) y sin edición de perfil/subida de foto real — quedan para otra iteración, los endpoints ya existen en el backend.
  - `shared/ui/avatar/` genera el avatar por defecto con DiceBear (`@dicebear/core` + `@dicebear/styles`, estilo "gaze") cuando no hay foto real (`hasAvatar` es `false` para todo el mundo hoy, al no existir subida). **Carga perezosa con `import()` dinámico dentro del propio componente** — no una importación estática — porque `AppHeader` (que monta el avatar) es eager, y un primer intento con importación estática metió ~170 kB extra en el bundle inicial (`@dicebear/core` completo, sin tree-shaking real: es autocontenido, sin dependencias, con su propio validador de JSON Schema). Con la carga perezosa, `@dicebear/core` y `gaze.json` son sus propios chunks lazy; el presupuesto de `angular.json` se quedó en 555 kB sin tocar — eliminar `session-menu` (ver más abajo) compensó de sobra los dos iconos nuevos y el propio componente.
  - **`layout/session-menu/` se eliminó el mismo día**, a pedido explícito: el cajón de tres puntos (tema + cerrar sesión, solo móvil/tablet) se consolidó en `/profile`, alcanzable desde el avatar del header en todos los tamaños. `ThemeToggle` volvió a ser icon-only —perdió su variante `menu`, que era el único consumidor de `.menu-item` fuera de `/profile`—. El historial de diseño del `<dialog>` modal queda documentado en [Decisiones abiertas](#decisiones-abiertas) por si hace falta un cajón similar en el futuro, marcado como superado.
  - La excepción direccional de `home/` (ver [Folder placement](#folder-placement--core-layout-shared-features)) se generalizó para cubrir también `features/user/profile/`, que importa `HabitService`/`TaskService` por el mismo motivo que Home: agregar dominios ajenos para mostrarlos, nunca poseer su lógica.
- **Header sin acciones inline, aviso de notificaciones tras el onboarding, y `/profile` como cajón animado** (2026-09-13, mismo día, rondas posteriores):
  - `app-header` perdió su bloque de acciones en escritorio (`ThemeToggle` + "Cerrar sesión"): ahora es marca + avatar en **todos** los tamaños, sin excepción. `layout/theme-toggle/` se eliminó por completo (quedó sin ningún consumidor tras esto).
  - El avatar dejó de ser un `routerLink` simple: es un `<button>` (`onAvatarClick()`) que decide navegar a `/profile` o volver a `/` según la ruta actual — un `routerLink` a la misma URL no navega, y un segundo clic estando ya en `/profile` tiene que poder "cerrarlo".
  - **`/profile` anima como un cajón que entra desde la derecha** (`.page` con `translateX`) cada vez que se monta, y **sale hacia la derecha antes de destruirse** vía `profileExitGuard` (`canDeactivate`), que llama a `Profile.animateOut()` — el componente pone `closing()` en `true` y el guard retiene la navegación hasta que la animación termina (o hasta 0ms si `prefersReducedMotion()`). Esto se dispara sin importar cómo se abandona la ruta (botón, atrás del navegador, otro enlace), no solo con el avatar.
  - **`NotificationPrompt`** (`layout/notification-prompt/`) pide activar push tras el login mientras `push.status()` siga en `'default'`; si se descarta, se recuerda en `localStorage` (`notification-prompt-storage.ts`) y no vuelve a insistir. Perfil gana un botón "Probar" (junto al switch, visible si `push.subscribed()`) que llama a `push.showTest()` — mismo patrón que `habit-list`.
  - **Se enganchó al final del onboarding de Inicio, no a "recién autenticado"**: mostrar un `<dialog>` de permiso a la vez que el tour de `driver.js` (los dos overlays de pantalla completa) se apilaba mal. `TourRunnerService` gana `finished$` (emite el `tourId` cuando ESE tour termina de verdad — completarlo, cerrarlo con X/Escape/velo; `stop()`, interrupción forzada, no cuenta) e `isActive(tourId)`. `NotificationPrompt` sigue mostrándose de inmediato para quien ya vio el tour `home` en otra sesión (`hasSeenTour('home')` siembra la condición) — solo cambia el caso de quien lo ve por primera vez.
  - **El tour `home` se encadena explícitamente detrás de `shell`** (`injectTourOnMount('home', steps, 'shell')`): antes competían por el cerrojo global de `TourRunnerService` y normalmente ganaba `shell`, dejando `home` pendiente hasta la SEGUNDA visita a Inicio — que habría retrasado el aviso de notificaciones igual de tarde. `injectTourOnMount` acepta un `waitFor` opcional: si ese tour sigue activo, se suscribe a `finished$` con `takeUntilDestroyed()` (si el componente se desmonta antes de que `waitFor` termine, se cancela — no dispara un tour más tarde sobre una pantalla que ya no es la suya); si no está activo (ya visto, o perdió su propia carrera), se intenta de inmediato, igual que antes.
  - Perfil marca la fila de notificaciones con una insignia (`badge badge--primary`, "Pendiente") cuando la opción sigue disponible (`status() !== 'denied' && !subscribed()`), para que resalte como acción pendiente en vez de depender solo del texto de estado.
- **Editar perfil** (2026-09-13) — `PUT /api/users/me` consumido por primera vez. `features/user/user.service.ts` (nuevo, primer servicio de la feature): `updateProfile()`, `uploadAvatar()`, `deleteAvatar()`, y el signal `avatarUrl` (blob local del avatar real, derivado de `hasAvatar` con `toObservable`+`switchMap`+`toSignal` — no un `effect()` escribiendo un signal, que es el antipatrón que la sección de Angular 20.3 prohíbe). `AuthStateService.applyUser()` deja aplicar el `UserResponse` que ya devuelve `PUT /me` sin una petición de más.
  - **Verificado contra el backend real (`/v3/api-docs`) antes de construir nada**: el mockup mostrado traía "Nombre de usuario" y "Número de teléfono", pero `ProfileUpdateRequest` solo tiene `firstName`/`lastName` — no existe ningún campo de usuario ni teléfono en ningún endpoint. Se quitaron del formulario en vez de dejarlos como placeholder muerto. El correo es de solo lectura esta vez (cambiarlo es `PUT /me/email`, con contraseña actual — mismo patrón de "configurar después" que la contraseña, que tiene su propio `PUT /me/password` sin consumir aún). Tampoco hay ninguna insignia de "correo verificado": el backend no tiene ese concepto en absoluto, así que no se fabrica uno falso.
  - **`GET /api/users/me/avatar` exige `Authorization`**, así que no puede ser un `<img src="/api/...">` a pelo (una etiqueta `<img>` no lleva cabeceras). `UserService.avatarUrl` lo trae como `blob` y expone un blob URL local; se revoca el anterior antes de pedir uno nuevo. `shared/ui/avatar/` (`Avatar`) ya tenía el input `photoUrl` preparado para esto desde que se construyó — cero cambios ahí.
  - Pantalla nueva `features/user/profile-edit/` (ruta `/profile/edit`, guardada). Misma técnica de una sola plantilla + `grid-template-areas` que `/profile`: una columna apilada en móvil (foto → info → seguridad → guardar), 2 columnas en escritorio. El botón "Guardar Cambios" vive en su propia área de rejilla (columna derecha, debajo de "Información Personal"), fuera del `<form>` visualmente pero conectado a él vía `form="profile-edit-form"` (atributo HTML, no un `(click)` duplicado) — así Intro sigue enviando desde los campos de texto aunque el botón esté en otra parte del DOM/rejilla.
  - **"Cambiar contraseña" es una fila inerte** (`aria-disabled`, insignia "Próximamente"), no un enlace muerto ni un botón que navega a ningún sitio — se cablea explícitamente en otra iteración.
  - **El engranaje del header vive en `AppHeader`, no en `/profile`**: `showSettings = computed(() => currentUrl().startsWith('/profile'))`, con `currentUrl` como `toSignal` de `router.events` filtrado a `NavigationEnd`. Solo aparece dentro de `/profile` (y `/profile/edit`, que comparte el prefijo) — en Inicio u otra feature no se pinta. `AppHeader` inyecta `UserService` (una feature) por el mismo motivo ya documentado para `pending-delete-confirm/`: el shell necesita mostrar un dato real de una feature, no reusar su componente.
  - `.profile-section-title` y `.profile-stack` subieron de `features/user/profile/` a clases globales (`.section-title`, `.profile-stack`) en `_components.scss` al llegar su segundo consumidor (`profile-edit/`) — mismo criterio que ya fijó `.menu-item`.
  - Presupuesto de `angular.json` subido de 555 kB a 560 kB: `UserService` (nuevo, con sus operadores de rxjs) y el seguimiento reactivo de la URL en `AppHeader` (eager, montado siempre) entran en el bundle inicial.
  - **Verificado end-to-end con Playwright contra el backend real**: registro → engranaje ausente en Inicio, presente en `/profile` → clic navega a `/profile/edit` con los campos precargados → guardar cambia `firstName` y se refleja en `/profile` al volver → subir una imagen de prueba reemplaza el avatar generado por uno real (`blob:`) y hace aparecer "Eliminar foto" → borrarla lo hace desaparecer y el avatar vuelve al generado por DiceBear.
- Recuperación y cambio de contraseña (2026-09-16) — frontend construido sobre el contrato de [Recuperación de contraseña](#recuperación-de-contraseña-authforgot-password-authreset-password--2026-09-14) y `PUT /me/password`:
  - `features/auth/forgot-password/` (`/forgot-password`, pública) y `features/auth/reset-password/` (`/reset-password?token=`, pública, `token` leído con `input()` vía `withComponentInputBinding()`, no `ActivatedRoute`). `reset-password` distingue "enlace inválido/caducado" (`400` sin `errors`) de validación de campos (`400` con `errors`) en el propio componente, no en `extractErrorMessage` — decisión de presentación del llamante, como ya documenta [Frontend architecture decisions](#frontend-architecture-decisions). Éxito → `router.navigate(['/login'], { queryParams: { passwordReset: '1' } })`; `Login` pinta un banner `.form-success` cuando ese query param está presente, y gana un enlace "¿Olvidaste tu contraseña?" en la misma fila que la etiqueta del campo (`.field-label-row`, patrón GitHub/Google) en vez de un párrafo centrado entre el campo y el botón.
  - **`core/auth/password-policy.ts`** (`PASSWORD_RULES`, `passwordPolicy`, `passwordsMatch`) — movido desde `features/auth/password-policy.ts` el mismo día en que `profile-edit` (otra feature) lo necesitó para su propio cambio de contraseña: regla de [Folder placement](#folder-placement--core-layout-shared-features), nada dentro de una feature se importa desde otra, la pieza compartida sube. Tres consumidores hoy: `register`, `reset-password`, `profile-edit`.
  - **`.field-checks*`** (lista de requisitos de contraseña con icono + colapso al foco) subió de un partial de Sass en `features/auth/` a clase global en `_components.scss` por el mismo motivo — cruzó de feature (`auth` → `user`), mismo criterio que ya fijó `.menu-item`/`.card-metric`.
  - **`profile-edit`**: la fila inerte "Cambiar contraseña" (`aria-disabled`, insignia "Próximamente") se reemplazó por un formulario funcional dentro de la misma tarjeta `.profile-edit-grid__security`, consumiendo `UserService.changePassword()` (`PUT /api/users/me/password`, nuevo). Formulario independiente del de "Información Personal" (submit propio, no el botón "Guardar Cambios" de la rejilla): son dos endpoints distintos con sus propios estados de envío/error/éxito. Campos `currentPassword`/`newPassword`/`confirmPassword`, mismo markup de `field-checks` que `reset-password`. Al tener éxito no hay `UserResponse` que aplicar ni sesión que rehidratar — el backend no invalida el JWT actual ni otras sesiones al cambiar la contraseña (misma regla que el reset por correo) — así que solo se muestra un banner `.form-success` y se limpia el formulario (`reset()`), sin navegar.
  - **Sin verificar contra el backend real todavía** (a diferencia del resto de `profile-edit`, que sí tiene una pasada de Playwright): falta probar `currentPassword` incorrecto (`400`, `errors: null`, mensaje "La contraseña actual es incorrecta." — cae al camino genérico de `extractErrorMessage`, sin caso especial como en `reset-password`) y el flujo feliz completo.
...
5. Env vars de Resend (`ALIGN_RESEND_API_KEY`/`ALIGN_EMAIL_FROM`/`ALIGN_FRONTEND_BASE_URL`) pendientes en el VPS antes del próximo deploy — sin ellos el backend no arranca en prod. El frontend de recuperación/cambio de contraseña ya está construido (ver [Current status](#current-status)); esto es puramente operativo.
